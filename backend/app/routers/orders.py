...
def create_order(payload: OrderCreate, db: Session = Depends(get_db)):
    # Run validation and updates inside a single transaction and lock product rows
    try:
        with db.begin():
            ...
            for item in payload.items:
                ...
                if product.stock_quantity < item.quantity:
                    raise HTTPException(
                        status_code=409,
                        detail=(
                            f"Insufficient stock for product '{product.name}'. Available: {product.stock_quantity}. Requested: {item.quantity}."
                        ),
                    )
            ...
            # exiting `with db.begin()` will commit if no exception occurred
    except IntegrityError as exc:
        # transaction has been rolled back by the context manager
